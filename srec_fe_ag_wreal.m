function [vad, silence, features, wave_est, dif_mul, per80, per20,Dif0,Dif0_real,KK] = srec_fe_ag_wreal(wave,cwave, dif_mul, step, step0, per80, per20,Dif0,Dif0_real)
alpha_n = 0.80;
alpha_s = 0.55;
...
for i = 1:n_frames;
    ...
    cframe = cwave((i-1) * frame_period + (1:window_length),:);
    
    windowed = frame .* kron(ones(1, Nmic), window);
    cwindowed = cframe .* kron(ones(1, Nmic), window);
    
    fd_frame = fft(windowed, params.np); 
    fd_cframe = fft(cwindowed, params.np); 
    %fd_nframe = fft(windowed-cwindowed, params.np); 
    half_fd_frame = fd_frame(1:params.np / 2 + 1, :);% .* kron(ones(1, Nmic), band); 
    half_fd_cframe = fd_cframe(1:params.np / 2 + 1, :);
    %half_fd_nframe = fd_nframe(1:params.np / 2 + 1, :);
    
    summ = mean(half_fd_frame, 2);
    csumm = mean(half_fd_cframe, 2);    

    phase = phase_next;
    phase_next = summ ./ (abs(summ) + eps);
    
    mag_sqr = mag_sqr_next;
    mag_sqr_real = mag_sqr_next_real;
    mag_sqr_dif = mag_sqr_dif_next;
    sum0 = abs(summ) .^ 2;
    csum0 = abs(csumm).^2;
    mag_sqr_next = 4*sum0;
    mag_sqr_next_real = 4*csum0;
    dif0 = zeros(257, 1);
    dif0_real = zeros(257,1);
    ss = 0;
    dd = 0;
    
    for ii = 1:Nmic-1,
        for j = ii+1:Nmic,
            dd_ij = abs(half_fd_frame(:,ii) - half_fd_frame(:,j)) ;
             %c
            ss_ij = abs(half_fd_frame(:,ii) + half_fd_frame(:,j)) ;
            dif0 = dif0 + dd_ij;
    ...
    for ii = 1:Nmic
        cdd_ij = abs(half_fd_cframe(:,ii));
        dif0_real = dif0_real +  cdd_ij;
    end
    
    dif0_real = dif0_real./5;
   
    Dif0(end+1) = {dif0};
    Dif0_real(end+1) = {dif0_real};
    ...
    ppreal = 0.5 * (mag_sqr_real + mag_sqr_next_real);
   
    phip = alpha_s * phip + (1 - alpha_s) * pp;
    phipreal = alpha_s * phipreal + (1 - alpha_s) * ppreal;
    ...
    wreal = min(1-eps, max(eps, phipreal)./(abs(phip) + eps));
    nsr = (wiener .^ -1 - 1);
   
   
    % if ~flag 
        
        %figure(1); plot([wiener, wreal, abs(wiener-wreal)]);
        %figure(2); plot(wave(:,1)); hold on; plot((i-1) * frame_period + (1:window_length), frame(:,1)); hold off;
        %disp(' ');
   %  end
...
wave_est = zeros((n_frames) * frame_period, 1);
for  i = 1:n_frames-1,
        af_half = summ_all(:, i);
